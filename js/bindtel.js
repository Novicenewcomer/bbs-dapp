...
var phonead=$('.bindtelvalue').mPicker({
	level:1,
	dataJson:adnumdata,
	Linkage:false,
	rows:6,
	...
	confirm:function(json){
		...
		registeredfn.read_telad();
	},
	cancel:function(json){
	}
})
var registeredfn = {
	thisdom:$(".bindtelboxcon"),
	thisdata:{"telad":"+86","telnum":"","yzm":""},
	btnd:false,
	init:function(){
		var _this = this;
		this.read_telad();
		this.thisdom.find(".btel_telvalue").on("input propertychange",function(){
			_this.thisdata.telnum =$(this).val()
			_this.readbindingbtn();
		})
		this.thisdom.find(".btel_yzmvalue").on("input propertychange",function(){
			_this.thisdata.yzm = $(this).val()
			_this.readbindingbtn();
		})
	},
	showconw:function(){
	...
	read_telad:function(){
		$("#bindtelvalue").val(this.thisdata.telad)
		this.thisdom.find(".bindtellabel span").html(this.thisdata.telad)
	},
	readbindingbtn:function(){
		var kg = true;
		this.btnd = false;
		for(var i in this.thisdata){
			if($.trim(this.thisdata[i])==""){
				kg = false;
			}
		}
		if (!kg) {
			$(".btel_bindingbtn").addClass("graybtn").removeClass("yellowbtn");
			return false;
		}
		this.btnd = true;
		$(".btel_bindingbtn").addClass("yellowbtn").removeClass("graybtn");
